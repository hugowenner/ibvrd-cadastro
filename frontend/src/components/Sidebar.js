...
const Sidebar = () => {
  ...
  const baseClasses =
    "flex items-center py-3 md:py-4 px-6 md:px-8 border-l-4 transition-all duration-300 group relative overflow-hidden";
  const activeClasses =
    "border-amber-600 bg-amber-50 text-amber-900 font-bold shadow-inner";
  const inactiveClasses =
    "border-transparent text-gray-500 hover:bg-white hover:text-amber-700 hover:border-gray-200 hover:shadow-sm";
    ...
    <>
      ...
      >
        ...
        <div className="p-6 md:p-8 border-b border-gray-50 mt-10 md:mt-0">
          ...
          <p className="pl-5 text-[10px] md:text-xs text-gray-400 mt-2 uppercase tracking-[0.2em] font-medium">
            Administração
          </p>
        </div>
          ...
          {user?.role === 'admin' && (
            <NavLink to="/usuarios" className={linkClasses} onClick={() => setIsOpen(false)}>
              ...
              <span className="ml-4 font-serif text-sm md:text-sm tracking-wide">Gerenciar Usuários</span>
            </NavLink>
          )}
